var causesOfDeathHierarchies = [
    {
        "id": 295,
        "children": [
            {
                "id": 955,
                "name": "HIV/AIDS and sexually transmitted infections",
                "childrenIds": [
                    948,
                    949,
                    950,
                    300,
                    298,
                    394,
                    395,
                    396,
                    397,
                    398,
                    399,
                    393,
                    955
                ]
            },
            {
                "id": 956,
                "name": "Respiratory infections and tuberculosis",
                "childrenIds": [
                    954,
                    934,
                    946,
                    947,
                    297,
                    322,
                    328,
                    329,
                    956
                ]
            },
            {
                "id": 957,
                "name": "Enteric infections",
                "childrenIds": [
                    302,
                    319,
                    320,
                    958,
                    959,
                    321,
                    957
                ]
            },
            {
                "id": 344,
                "name": "Neglected tropical diseases and malaria",
                "childrenIds": [
                    345,
                    346,
                    348,
                    349,
                    347,
                    350,
                    351,
                    352,
                    353,
                    354,
                    355,
                    356,
                    357,
                    358,
                    359,
                    361,
                    362,
                    363,
                    360,
                    364,
                    405,
                    843,
                    935,
                    936,
                    365,
                    344
                ]
            },
            {
                "id": 961,
                "name": "Other infectious diseases",
                "childrenIds": [
                    332,
                    337,
                    338,
                    339,
                    340,
                    341,
                    342,
                    401,
                    402,
                    403,
                    404,
                    400,
                    408,
                    961
                ]
            },
            {
                "id": 962,
                "name": "Maternal and neonatal disorders",
                "childrenIds": [
                    367,
                    368,
                    369,
                    370,
                    995,
                    374,
                    375,
                    376,
                    741,
                    379,
                    366,
                    381,
                    382,
                    383,
                    384,
                    385,
                    380,
                    962
                ]
            },
            {
                "id": 386,
                "name": "Nutritional deficiencies",
                "childrenIds": [
                    387,
                    388,
                    389,
                    390,
                    391,
                    386
                ]
            }
        ],
        "name": "Communicable, maternal, neonatal, and nutritional diseases",
        "parentId": 294
    },
    {
        "id": 409,
        "children": [
            {
                "id": 410,
                "name": "Neoplasms",
                "childrenIds": [
                    444,
                    447,
                    450,
                    411,
                    414,
                    441,
                    418,
                    419,
                    420,
                    996,
                    1021,
                    417,
                    453,
                    456,
                    423,
                    426,
                    459,
                    849,
                    850,
                    462,
                    429,
                    432,
                    435,
                    465,
                    438,
                    468,
                    471,
                    474,
                    477,
                    480,
                    483,
                    484,
                    485,
                    486,
                    845,
                    846,
                    847,
                    848,
                    943,
                    487,
                    1022,
                    964,
                    965,
                    966,
                    967,
                    490,
                    410
                ]
            },
            {
                "id": 491,
                "name": "Cardiovascular diseases",
                "childrenIds": [
                    492,
                    493,
                    495,
                    496,
                    497,
                    494,
                    498,
                    968,
                    969,
                    970,
                    504,
                    942,
                    938,
                    944,
                    499,
                    500,
                    501,
                    502,
                    503,
                    1023,
                    491
                ]
            },
            {
                "id": 508,
                "name": "Chronic respiratory diseases",
                "childrenIds": [
                    509,
                    511,
                    512,
                    513,
                    514,
                    510,
                    515,
                    516,
                    520,
                    508
                ]
            },
            {
                "id": 526,
                "name": "Digestive diseases",
                "childrenIds": [
                    522,
                    523,
                    524,
                    971,
                    525,
                    521,
                    527,
                    528,
                    536,
                    992,
                    529,
                    530,
                    531,
                    532,
                    533,
                    534,
                    535,
                    541,
                    526
                ]
            },
            {
                "id": 542,
                "name": "Neurological disorders",
                "childrenIds": [
                    543,
                    544,
                    545,
                    546,
                    554,
                    547,
                    548,
                    972,
                    557,
                    542
                ]
            },
            {
                "id": 558,
                "name": "Mental disorders",
                "childrenIds": [
                    559,
                    568,
                    569,
                    567,
                    570,
                    571,
                    573,
                    574,
                    572,
                    575,
                    578,
                    579,
                    582,
                    585,
                    558
                ]
            },
            {
                "id": 973,
                "name": "Substance use disorders",
                "childrenIds": [
                    560,
                    562,
                    563,
                    564,
                    565,
                    566,
                    561,
                    973
                ]
            },
            {
                "id": 974,
                "name": "Diabetes and kidney diseases",
                "childrenIds": [
                    975,
                    976,
                    587,
                    997,
                    998,
                    591,
                    592,
                    593,
                    589,
                    588,
                    974
                ]
            },
            {
                "id": 653,
                "name": "Skin and subcutaneous diseases",
                "childrenIds": [
                    977,
                    978,
                    979,
                    654,
                    655,
                    656,
                    657,
                    980,
                    658,
                    659,
                    660,
                    661,
                    662,
                    663,
                    664,
                    665,
                    668,
                    653
                ]
            },
            {
                "id": 669,
                "name": "Sense organ diseases",
                "childrenIds": [
                    670,
                    671,
                    672,
                    999,
                    1000,
                    675,
                    981,
                    674,
                    679,
                    669
                ]
            },
            {
                "id": 626,
                "name": "Musculoskeletal disorders",
                "childrenIds": [
                    627,
                    1014,
                    1015,
                    1016,
                    1017,
                    628,
                    630,
                    631,
                    632,
                    639,
                    626
                ]
            },
            {
                "id": 640,
                "name": "Other non-communicable diseases",
                "childrenIds": [
                    642,
                    643,
                    644,
                    645,
                    646,
                    647,
                    648,
                    649,
                    650,
                    651,
                    652,
                    641,
                    595,
                    596,
                    597,
                    598,
                    602,
                    594,
                    604,
                    605,
                    606,
                    607,
                    608,
                    609,
                    612,
                    603,
                    614,
                    837,
                    615,
                    838,
                    616,
                    839,
                    618,
                    613,
                    619,
                    681,
                    682,
                    683,
                    684,
                    685,
                    680,
                    686,
                    640
                ]
            }
        ],
        "name": "Non-communicable diseases",
        "parentId": 294
    },
    {
        "id": 687,
        "children": [
            {
                "id": 688,
                "name": "Transport injuries",
                "childrenIds": [
                    690,
                    691,
                    692,
                    693,
                    694,
                    689,
                    695,
                    688
                ]
            },
            {
                "id": 696,
                "name": "Unintentional injuries",
                "childrenIds": [
                    697,
                    698,
                    699,
                    701,
                    703,
                    700,
                    705,
                    707,
                    704,
                    708,
                    710,
                    711,
                    709,
                    713,
                    714,
                    715,
                    712,
                    842,
                    729,
                    716,
                    696
                ]
            },
            {
                "id": 717,
                "name": "Self-harm and interpersonal violence",
                "childrenIds": [
                    721,
                    723,
                    718,
                    725,
                    726,
                    941,
                    727,
                    724,
                    945,
                    854,
                    717
                ]
            }
        ],
        "name": "Injuries",
        "parentId": 294
    },
    {
        "id": 1029,
        "children": [],
        "name": "Total cancers",
        "parentId": 294
    },
    {
        "id": 1026,
        "children": [],
        "name": "Total burden related to hepatitis B",
        "parentId": 294
    },
    {
        "id": 1027,
        "children": [],
        "name": "Total burden related to hepatitis C",
        "parentId": 294
    },
    {
        "id": 1028,
        "children": [],
        "name": "Total burden related to Non-alcoholic fatty liver disease (NAFLD)",
        "parentId": 294
    }
]


const csv = require('csv-parser');
const fs = require('fs');

var map = {};
fs.createReadStream('Causes_Of_Death.csv')
    .pipe(csv())
    .on('data', (row) => {
        if(row.age_id !== "27" || row.sex_id !== "3") return;
        if (map[row.location_name]) {
            var countrySummary = map[row.location_name];
            if (countrySummary[row.year]) {
                yearSummary = countrySummary[row.year];
                updateCausesData(yearSummary, row);
            }
            else {
                let arrData = JSON.parse(JSON.stringify(causesOfDeathHierarchies)); 
                updateCausesData(arrData, row);
                countrySummary[row.year] = arrData;
            }
        }
        else {
            let arrData = JSON.parse(JSON.stringify(causesOfDeathHierarchies)); 
            updateCausesData(arrData, row);
            map[row.location_name] = {}
            map[row.location_name][row.year] = arrData;
        }
    })
    .on('end', () => {
        let json = JSON.stringify(cleanupFields(map));
        fs.writeFile('causes-of-death-data.json', json, (err) => {
            if (err) {
                throw err;
            }
            console.log("JSON data is saved.");
        });
    })



function updateCausesData(hierarchyData, row) {
    let rowId = parseInt(row.cause_id);
    let element = hierarchyData
        .filter(x => x.id === rowId
            || x.children.map(y => y.id).includes(rowId)
            || x.children.map(y => y.childrenIds).flat().includes(rowId))[0]
    if (element == null) { console.log("error"); return; }
    if (!element.sum) element.sum = 0;
    element.sum += parseFloat(row.val);
    if (element.id !== rowId) {
        let childElement = element.children.filter(x => x.id === rowId || x.childrenIds.includes(rowId))[0];
        if (!childElement.sum) childElement.sum = 0;
        childElement.sum += parseFloat(row.val);
    }
}

function cleanupFields(data) {
    for (const [key, value] of Object.entries(data)) {
        for (const [key2, value2] of Object.entries(value)) {
            console.log(`${key2}: ${value2}`);
            for (let index = 0; index < value2.length; index++) {
                const element = value2[index];
                for (let index2 = 0; index2 < element.children.length; index2++) {
                    const element2 = element.children[index2];
                    delete element2.childrenIds;
                }
            }
        }
    }
    return data
}